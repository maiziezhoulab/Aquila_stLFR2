...
S=$8
O=$9
other_vc_flag=${10} ## --dipcall; --delly
# dist= 500 p = 0.5 P = 0.5 S = 30 O = 0.01
...
truvari bench -b /data/maiziezhou_lab/CanLuo/long_reads_project/Benchmarks/HG002_SVs_Tier1_v0.6_chr${chr_num}_del.vcf.gz -c $work_dir/${prefix}_DEL_noXY_sorted.vcf.gz -f ${ref} -o $output_dir/DEL_50_ --includebed ${bed} -p $p -P $P -r $dist --passonly --sizemin 50 -S ${S} -O ${O}

truvari bench -b /data/maiziezhou_lab/CanLuo/long_reads_project/Benchmarks/HG002_SVs_Tier1_v0.6_chr${chr_num}_ins.vcf.gz  -c $work_dir/${prefix}_INS_noXY_sorted.vcf.gz -f ${ref} -o $output_dir/INS_50_  --includebed ${bed} -p $p -P $P -r $dist --passonly --sizemin 50  -S ${S} -O ${O}
