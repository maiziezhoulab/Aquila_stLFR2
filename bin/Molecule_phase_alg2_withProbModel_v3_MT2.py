...
def Get_h5_for_mole_with_one_hetero_var(h5_file,chr_num,output_dir):
    ...
    logger.info("Extract molecule with one heterozygous variant finished...")
...
def Recursive_Clustering_for_Overlap_Variants(final_cluster,step,h5_filename,overlap_var_threshold):
    ...
    if len(cluster_list) == prev_cluster_num:
        ...
        logger.info("**** Iteration " +  str(step) + " **** ")
        logger.info("# non-merge event: " + str(count_nonmerge))
        logger.info("Previous cluster size: %d current cluster size: %d prev size - cur size = %d"%(prev_cluster_num,len(cluster_list), prev_cluster_num - len(cluster_list)))
        logger.info("Converged.")
        logger.info("Second round clustering finished.")
    ...
    step += 1
    if step == 1:
        logger.info("Second round clustering start...")
        logger.info("**** Iteration " +  str(step) + " **** ")
        logger.info("# non-merge event: " + str(count_nonmerge))
        logger.info("Previous cluster size: %d"%prev_cluster_num)
        logger.info("# non-merge / prev size = " + str(round(float(count_nonmerge/prev_cluster_num),3)) )
    else:
        logger.info("**** Iteration " +  str(step) + " **** ")
        logger.info("# non-merge event: " + str(count_nonmerge))
...
def Recursive_clustering(all_merge_cluster_dict,all_merge_cluster_complement_dict,all_merge_cluster_dict_start_end, all_merge_cluster_complement_dict_start_end,step,mole_dict,mole_info,h5_filename,overlap_var_threshold,support_threshold):
    ...
    logger.info("Total num: " + str(total_num))
    logger.info("cluster size: " + str(len(cluster_dict)))
    logger.info("Complementary cluster size: " + str(len(cluster_complement_dict)))
    logger.info("total num - cluster size = " + str(total_num - len(cluster_dict)))

    if prev_num == len(all_merge_cluster_dict):
        logger.info("Converged...")
        logger.info("merged cluster size: " + str(len(all_merge_cluster_dict)))
        logger.info("First round clustering finished.")
    ...
    for key,mole_num_list in all_merge_cluster_dict.items():
        if use_cluster_dict[key] != 1:
            ...
            if merge_cluster_flag == 0:
                ...
                cluster_complement_dict_start_end[cluster_num] = [cluster_complement_start, cluster_complement_end]
                cluster_num += 1
    logger.info("First round clustering start...")
    logger.info("**** Iteration " + str(1) + " ****")
...
def Phase_start(output_dir,h5_dir,sample_name,chr_start,chr_end,overlap_var_threshold,support_threshold,num_threads, xin):
    ...
    logger.info("using overlap variants threshold: " + str(overlap_var_threshold)) 
    logger.info("using molecules support threshold: " + str(support_threshold)) 
    ...
    for chr_num in range(chr_start,chr_end+1):
        logger.info("Process chr" + str(chr_num) + "...")
        ...
        logger.info(""" Assign phase block for molecule with only one heterzygous variant""")
        ...
        logger.info(""" Impute the phase block for the rest very few molecules """)
