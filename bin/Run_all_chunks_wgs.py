...
parser.add_argument('--qnames_dir','-q', help="Directory to store qname files")
...
def Run_all_chunks(out_dir,out_dir_prefix,qnames_dir,sample_name,num_of_threads,chr_start,chr_end):
    ...
    for one_file in fastq_files_all:
        ...
        cmd_used = "python3 " + code_path + "Split_barcoded_fastq_by_chr_all_by_smallchunks_wgs.py --chr_start " + str(chr_start) + " --chr_end " + str(chr_end) + " --out_dir " + out_dir_2 + " --fastq_file " + one_file + " --qnames_dir " + qnames_dir + " --sample_name " + sample_name  
...
#        os.makedirs(out_dir)
 ...
 #   Popen(split_cmd,shell=True).wait()
    Run_all_chunks(out_dir,out_dir_prefix,qnames_dir,sample_name,num_of_threads,chr_start,chr_end)
