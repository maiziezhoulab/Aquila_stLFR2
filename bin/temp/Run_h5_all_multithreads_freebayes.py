...
parser.add_argument('--read_type','-r',help = 'SE/PE')
...
def process_sorted_bam(bam_file,output_file,qname_file,qname_pos_file,variant_dict,threshold_start,threshold_end,boundary,mmq_threshold,read_type):
    ...
    for barcode,value in tqdm(mole_dict.items(),desc = 'loop through molecules'):
        if len(value) > 0:
            count_del = 0
...
def Run_h5_all(bam_file,vcf_file,chr_start,chr_end,sample_name,mbq_threshold,mmq_threshold,boundary,num_threads,out_dir,variant_dict,read_type):
    count = 1
    ...
    for key, val in run_list.items():
        ...
        for chr_num in val:
            ...
            pool.apply_async(process_sorted_bam,(bam_file,h5_file,h5_qname_file,h5_qname_pos_file,variant_dict,chr_num,chr_num,boundary,mmq_threshold,read_type))
...
if __name__ == "__main__":
    ...
    assert read_type in ['SE','PE']
    Run_h5_all(bam_file,vcf_file,chr_start,chr_end,sample_name,mbq_threshold,mmq_threshold,boundary,num_threads,out_dir,variant_dict,read_type)
