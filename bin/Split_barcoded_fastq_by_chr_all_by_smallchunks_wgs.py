...
parser.add_argument('--qnames_dir','-q',help="directory for qname file")
...
def get_qname_dict(chr_start,chr_end,qnames_dir,sample_name):
    qname_dict = defaultdict(lambda: defaultdict(int))
    for chr_num in range(chr_start,chr_end+1):
        cur_pickle_file = qnames_dir + "/merged_chr" + str(chr_num) + ".txt"

        with open(cur_pickle_file,'r') as f:
            for line in f:
                qname = line[:-1]
...
if __name__ == "__main__":
    ...
    for chr_num in range(chr_start,chr_end + 1):
        qname_dict = get_qname_dict(chr_num,chr_num,qnames_dir,sample_name)
