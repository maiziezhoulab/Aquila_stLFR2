...
import os
from load_read_mem import retrieve_reads_mt_len
...
def extract_fq_from_fq(out_dir,n_ck,fqfile, index_dir, prefix,n_thread, out_prefix):
    # n_ck = 70
    cmd = "sort " + out_dir+'/unused_name_uniq.txt' + " > " + out_dir+'/unused_name_uniq_sorted.txt'
    print(cmd)
    Popen(cmd,shell= True).wait()

    unused_name_uniq  = out_dir+'/unused_name_uniq_sorted.txt'
    unused_names  = load_names(unused_name_uniq)
    chunks = cut_chunks(unused_names, n_ck, out_dir, prefix)
    print(len(unused_names))
    print(chunks)

    outfile_list = [  out_dir+f'/{prefix}_{ck[0]}_{ck[1]}.fq' for ck in chunks]
    temp_dir = out_dir+'/temp/'
    temp_prefix_list = [f'/{prefix}_{ck[0]}_{ck[1]}' for ck in chunks ]

    # i = 0

    # retrieve_reads_mt_len(unused_names[chunks[i][0]: chunks[i][1]],
    #                                                                      index_dir,fqfile, outfile_list[i],
    #                                                                      out_prefix,
    #                                                                      temp_dir, temp_prefix_list[i]) 

    sequences = Parallel(n_jobs=n_thread)(delayed(retrieve_reads_mt_len)(unused_names[chunks[i][0]: chunks[i][1]],
                                                                         index_dir,fqfile, outfile_list[i],
                                                                         out_prefix,
                                                                         temp_dir, temp_prefix_list[i])  
...
# extract_fq_from_fq(out_dir,n_ck,fqfile_r1, index_dir_r1, 'r1',n_thread, out_prefix)
# extract_fq_from_fq(out_dir,n_ck,fqfile_r2, index_dir_r2, 'r2',n_thread, out_prefix)
